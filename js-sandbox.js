...
const generateAdapter = (
	...
	return keywords
		.map(keyword => `${key} ${childProp} [@${checkProp} == '${keyword}']`)
		.join(`\n      ${logical} `)
}
	...
	return new Proxy(boxObject, {
		has: (target, key) => {
			if (!target.hasOwnProperty(key)) {
				throw new Error(
					`脚本引用了未插入的关键字：${key}，如需使用请通过 options.inject 配置插入。`,
				)
			}
			return target[key]
		},
...
function createSandbox(source, options = {}) {
	try {
		...
		disabledKeywords = getCleanKeywords(
			disabledKeywords,
			Object.keys(inject),
		)
		...
		if (!validate.ok) {
			console.warn(
				'脚本包含不被允许的关键字引用: ',
				Object.keys(validate.wordsState),
			)
			return
		}
