	...
	if (event.state.kuangMove) {
		...
		Object.values(platform.state.widgetAdded).forEach(v => {
			...
			if (v.scene === scene.state.index) {
				...
				if (
					minPointerX < widgetStartX && widgetStartX < maxPointerX
					&& minPointerY < widgetStartY && widgetStartY < maxPointerY
					&& minPointerX < widgetEndX && widgetEndX < maxPointerX
					&& minPointerY < widgetEndY && widgetEndY < maxPointerY
				) {
