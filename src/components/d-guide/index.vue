<template lang="pug">
  .d-guide
    ...
    ul.d-guide-right-menu(v-show="showGuideMenu" :style="`left: ${menuLeft}px; top:${menuTop - 10}px`")
  ...
  @Component
    ...
    get guideStyle () {
      return {
        ...
        top: `${this.platform.ruler.guideStartY < 0 ? 0 : this.platform.ruler.guideStartY + this.platform.ruler.size}px`
      }
    }
    ...
    handleGuideDrag (e, item) {
      ...
      this.platform.ruler.guideDragStartY = clientY - this.platform.ruler.yRoom
    ...
    lineStyle ({ type, site }) {
      const style = {}
      if (type === 'h') {
        style.top = `${this.platform.ruler.guideStartY < 0 ? site * this.platform.ruler.zoom +
            this.platform.ruler.guideStartY + this.platform.ruler.size
            : site * this.platform.ruler.zoom}px`
        style.left = `-${this.platform.ruler.guideStartX < 0 ? 0 : this.platform.ruler.guideStartX + this.platform.ruler.size}px`
      }
      if (type === 'v') {
        style.left = `${this.platform.ruler.guideStartX < 0 ? site * this.platform.ruler.zoom +
            this.platform.ruler.guideStartX + this.platform.ruler.size
            : site * this.platform.ruler.zoom}px`
        style.top = `-${this.platform.ruler.guideStartY < 0 ? 0 : this.platform.ruler.guideStartY + this.platform.ruler.size}px`
      }
      return style
    }
