	...
	<div class="single-query">
		...
		<div v-if="dataType == 0">
			<Form :model="querySingle" :label-width="90" class="single-query-form" ref="querySingle"
				...
				<FormItem :label="index > 0 ? '' : '条件'" :label-width="40"
						  v-for="(item, index) in querySingle.cond.itemList" :key="index">
					<Row :gutter="4" :style="index == 0 ? 'transform:translateX(-2px)' : ''">
						...
						<i-col :span="item.fieldCond == 'between' ? '3' : '5'">
							<i-select v-model="item.fieldName" transfer
						...
						<i-col span="2" style="display: flex">
							...
							<Icon type="md-remove-circle" @click="removeSingleCond(index)" class="btn"
