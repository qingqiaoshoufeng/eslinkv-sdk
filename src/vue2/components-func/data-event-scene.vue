...
d-right-swiper-list(
	...
	:list="editor.currentWidget.config.event.scene",
		...
		.d-manage-modal-control
			.fn-flex.flex-column(:style="{ width: '100%', paddingRight: '10px' }")
				i-select(
					...
					v-model="editor.currentWidget.config.event.scene[dataDefault.activeIndex].type",
					placeholder="事件类型")
				...
				i-select(
					:style="{ marginBottom: '10px' }",
					v-model="editor.currentWidget.config.event.scene[dataDefault.activeIndex].id",
					filterable,
					placeholder="目标场景")
				...
				i-select(
					...
					filterable,
					placeholder="场景过度动画",
					v-model="editor.currentWidget.config.event.scene[dataDefault.activeIndex].animate")
...
export default class FuncData extends func {
	...
	handleAddClick(): void {
		this.editor.currentWidget.config.event.scene.push({
			id: '',
			type: '',
			animate: '',
		})
	...
	handleRemoveClick(index: number): void {
		this.editor.currentWidget.config.event.scene.splice(index, 1)
	}
}
</script>
