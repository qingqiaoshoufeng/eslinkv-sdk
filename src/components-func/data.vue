<template lang="pug">
  .d-manage-modal-control-data
    ...
    .d-manage-modal-control(v-if="apiType==='自定义URL'")
      label
      .d-manage-modal-control-right
        i-select(v-model="item.config.api.method" :disabled="platform.chooseWidgetState" :style="{marginRight:'10px'}")
    ...
    .d-manage-modal-control(v-if="item.config.api.system.enable")
      label
      .d-manage-modal-control-right
        i-select(v-model="item.config.api.system.method" :disabled="platform.chooseWidgetState" :style="{marginRight:'10px'}")
          i-option(value="GET") GET
          i-option(value="POST") POST
          i-option(value="PUT") PUT
          i-option(value="DELETE") DELETE
    ...
    .d-manage-modal-control(v-if="item.config.api.autoFetch.enable")
      label
      .d-manage-modal-control-right
        i-input-number(:min="1" :step="1" :formatter="value => `${value} ms`"
          v-model="item.config.api.autoFetch.duration" :disabled="platform.chooseWidgetState")
    .d-manage-modal-control
      label 组件关联
      .d-manage-modal-control-right
  ...
  export default class FuncData extends func {
    ...
    get relateList () {
      const list = Object.values(this.platform.widgetAdded)
          .filter(v => (v as any).config.api.bind.enable && (v as any).scene === scene.state.index)
          .map(v => {
