      ...
      i-panel {{config.label}}
        .content(slot="content")
          .wrap(v-for="(v, n) in obj[inputKey]" :key="n")
            ...
            .fn-flex
              i-icon.pointer(:style="{marginLeft:'auto'}" title="删除" type="ios-trash-outline" color="#fff" size="14" @click="remove(n)")
          .fn-flex
            i-icon.pointer(:style="{marginLeft:'auto'}" title="新增" type="ios-add-circle-outline" color="#fff" size="14" @click="add")
</template>
<script lang="ts">
