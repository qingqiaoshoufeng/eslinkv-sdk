...
let state, actions
if (window.eslinkV.$store.platform) {
	state = window.eslinkV.$store.platform.state
	actions = window.eslinkV.$store.platform.actions
} else {
	...
	window.eslinkV.$store.platform = {state, actions}
}
